...
A = 2;
% Lagrangian Equation
f = 2*x*y+x^2*y+Lam*(x^2+y^2-2);
% f = x*y*(2+x)+Lam*(x^2+y^2-2);
dl = jacobian(f);
sol = solve(dl(1)==0,dl(2)==0,dl(3)==0);
sol = [eval(sol.x),eval(sol.y),eval(sol.Lam)]
sol = double(sol);

for i = 1:length(sol)
    if(isreal(sol(i,1))~=1 || isreal(sol(i,2))~=1 || isreal(sol(i,3))~=1)      
        sol(i,1:3) = NaN;       
    end 
end
...
xunit = r * cos(th) + x;
yunit = r * sin(th) + y;
zunit = xunit.^2+0.1+yunit.^2+0.1+1.5;
h = plot3(xunit, yunit,zunit,'r-',"MarkerSize", 20);
end
